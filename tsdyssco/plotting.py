...
def plot_envelopes(data):
    ...
    fig = tools.make_subplots(rows=3, cols=num_of_conditions, subplot_titles=[titlemaker(condition) for condition in condition_list],
    ...
    for col,condition in enumerate(condition_list):
        fig.append_trace(go.Scatter(x=list(data[condition_list[col]]['growth_rates']),
                                           y=list(data[condition_list[col]]['substrate_uptake_rates']),
                                           line={'color':colors[col]},name=condition,
